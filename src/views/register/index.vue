    ...
    >
      ...
      <el-form-item prop="username" class="mb-6 h-18">
        ...
        <el-input
          ...
          @keyup.enter.native="onSubmitForm('register_form')"
        />
      </el-form-item>

      <el-form-item prop="email" class="mb-6 h-18">
        <span class="svg-container">
      ...
      <el-form-item prop="confirm_password" class="mb-6 h-18">
        <span class="svg-container">
          <i class="el-icon-edit" />
        </span>
        <el-input
          v-model="formRequest.confirm_password"
      ...
      <el-button
        ...
        @click.native.prevent="onSubmitForm('register_form')"
        @keyup.enter.native="onSubmitForm('register_form')"
      >{{ $t("register") }}
      </el-button>
      ...
      <el-button
        ...
        style="margin: 0px;"
        @click.native.prevent="$router.push({ path: '/login' })"
      >{{ $t("login") }}
      </el-button>
    </el-form>
  </div>
...
import { validateText, validateEmail, validateConfirmPassword } from "@/utils/validate";
...
export default {
        ...
        confirm_password: [
          {
            validator: (rule, _, callback) =>
              validateConfirmPassword(
                rule,
                { password: this.formRequest.password, confirm_password: this.formRequest.confirm_password },
                callback,
  ...
  methods: {
    ...
    handleRegister() {
      this.$store
        .dispatch("user/register", this.formRequest)
        .then((res) => {
          if (res.statusCode === 200) { return window.location.reload(); }
